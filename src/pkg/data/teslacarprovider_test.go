...
func TestLoadConfigFromEnv1(t *testing.T) {
	...
	os.Setenv(TESLA_WAKEUP_TIMEOUT, "")
...
// Test for timeout default
func TestLoadConfigFromEnv4(t *testing.T) {
	...
	assert.Equal(t, config.FenceLatLong.Long, float64(-122.112782))
	assert.Equal(t, config.FenceRadius, float64(30))
	assert.Equal(t, config.WakeupTimeout, WakeupTimeoutSecondsDefault)
}

// Test for fence and wakeup defaults
func TestLoadConfigFromEnv5(t *testing.T) {
	...
	assert.Equal(t, config.FenceLatLong.Lat, float64(47.642744))
	assert.Equal(t, config.FenceLatLong.Long, float64(-122.112782))
	assert.Equal(t, config.FenceRadius, FenceRadiusMetersDefault)
	assert.Equal(t, config.WakeupTimeout, WakeupTimeoutSecondsDefault)
}

// Test for fence defaults
func TestLoadConfigFromEnv6(t *testing.T) {
	...
	os.Setenv(FENCE_LATLONG, LATLONG)
	os.Setenv(FENCE_RADIUS, "")
	os.Setenv(TESLA_WAKEUP_TIMEOUT, "50")
	...
	assert.Equal(t, config.FenceLatLong.Long, float64(-122.112782))
	assert.Equal(t, config.FenceRadius, FenceRadiusMetersDefault)
	assert.Equal(t, config.WakeupTimeout, int(50))
}
